...
from apps.movies.models import Movie, Price, Provider
...
def index(request):
    ...
    if request.method == "POST":
            ...
            try:
                ...
                for media_entry in media_entries:
                    ...
                    for offer in media_entry.offers:
                        if (
                            convert_tech_name_to_short(offer.package.technical_name)
                            in providers
                            or not providers
                        ):
                            matched = True
        ...
        country_iso = request.COOKIES.get(
            "country_iso", get_country_code_from_request(request)
        )
